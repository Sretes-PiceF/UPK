...
const Halpertama = () => {
    ...
    const fetchData = async () => {
        try {
            const result = await axios(`${process.env.NEXT_PUBLIC_API_URL}/api/ekstrakulikuler`)
    ...
    const deleteEkstrakulikuler = async (ekstrakulikuler_id: number) => {
        try {
            await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/ekstrakulikuler/${ekstrakulikuler_id}`);
            alert("Sukses menghapus");
            setEkstrakulikulerData((prevData) =>
                prevData.filter(item => item.ekstrakulikuler_id !== ekstrakulikuler_id)
            );
        } catch (error) {
                                        ...
                                        <td className="px-4 py-2 border">
                                            <div className="flex space-x-2">
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => deleteEkstrakulikuler(rs.ekstrakulikuler_id)}
                                                >
                                                    <Trash />
                                                </button>
                                            </div>
